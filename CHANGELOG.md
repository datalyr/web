...
- Heat mode records no rrweb session. It captures clicks (`{ t: 'click', ts, path, x_pct, y_pct, y_px, vw, vh, ph, text, sel, kind }`, with `kind` `rage` for 3+ clicks under a second apart within 30 px and `dead` when nothing on the page changed within 2.5 s; links and inputs are never dead), the deepest scroll per path (`{ t: 'scroll', ts, path, y_pct_max, ph }`, on tab hide, unload and SPA navigation), one attribution item per page load (the same fields as the replay marker), and on one page load in four a single masked DOM snapshot (`{ t: 'snap', ts, path, vw, snapshot }`, same masking as replay, dropped above 900 KB). `path` is the path only, never the query or fragment. Click `text` follows the replay text rules: only the text of buttons, links, labels, summaries, `[role=button]` and `[data-dl-unmask]`, never inside `[data-dl-mask]` or a form field, at most 80 characters. Sent with the replay transport to `replay.datalyr.com/replay?enc=gzip&mode=heat`, envelope plus `m: 'heat'`. Listening stops after 5 minutes without interaction and resumes on the next one; capture ends after 60 minutes on a page.
- The replay module grows from ~28 KB to ~37 KB gzip (rrweb-snapshot 2.1.6 for the heat snapshot). dl.js is unchanged apart from the mode decision.
